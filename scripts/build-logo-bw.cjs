...
<svg viewBox="0 0 500 500" width="100%" height="100%" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <filter id="naturalShadowBw" x="-10%" y="-10%" width="125%" height="125%">
  ...
  <g filter="url(#naturalShadowBw)">
    ...
    ${Array.from({ length: numOrange }).map((_, i) => {
      ...
      return `<circle cx="${hx}" cy="${hy}" r="${orangeHeadRadius}" fill="#6B7280" />`;
    }).join('\n    ')}
...
const bwSvg = generateBwSvg();
const publicDir = path.join(__dirname, '..', 'public');
fs.writeFileSync(path.join(publicDir, 'team-golden-star-logo-bw.svg'), bwSvg);

const distDir = path.join(__dirname, '..', 'dist');
if (fs.existsSync(distDir)) {
  fs.writeFileSync(path.join(distDir, 'team-golden-star-logo-bw.svg'), bwSvg);
}

console.log('Successfully written monochrome Team Golden Star Logo SVG');
